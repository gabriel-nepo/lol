...
export default function BasicTable(props) {
    ...
    const getData = async (date) => {
        if(props.match!== ''){
            const data = await fetch(`https://feed.lolesports.com/livestats/v1/details/${props.match}?startingTime=${date}`);
    ...
    React.useEffect(() => {
        ...
        date = date.split('.')[0] + "Z";
        date = date.replaceAt(18, '0');
        const interval = setInterval(() => getData(date), 20000);
        // getData();
    }, [props.match])
